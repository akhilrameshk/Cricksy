...
const canGenerateFinal = tossDone && lineupDone;
  useEffect(() => {
 ...
 const generateSuggestion = async (
  ...
  if (!canGenerateFinal) {
    setLockedMessage(
      "Please check toss and lineup first. Final fantasy team can be generated only after both are available."
    );
    return;
  }
  ...
  const res = await fetch("/api/fantasy-suggestion", {
    ...
    body: JSON.stringify({
      ...
      tossInfo,
      lineupInfo,
      secondBattingTeam: tossInfo?.secondBattingTeam,
      secondBowlingTeam: tossInfo?.secondBowlingTeam,
    }),
  });
  ...
  if (json.success) {
    ...
    setLockedMessage(json.message || "Failed to generate fantasy suggestion");
  }
            ...
            >
              <CardContent sx={{ p: 2 }}>
                ...
                <Typography sx={{ mt: 0.5, fontSize: 13, color: "#64748b" }}>
                  Normal team can be created before toss as dummy/pre-toss. It
                  will improve after toss and lineup are updated.
                </Typography>
{tossInfo && (
  <InfoBox
    label="Toss Check"
  ...
  {canCreateSecondInnings
    ...
    : "Second Innings Team Locked"}
                ...
                >
                  ...
                  >
                    {canCreateSecondInnings
                      ? `Second innings teams will use ${teamB} batters/all-rounders and ${teamA} bowlers/all-rounders only.`
